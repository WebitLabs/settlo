...
<x-filament-widgets::widget>
    <x-filament::section
        ...
        :description="$estimation ? 'This business\' share ('.$sharePercent.' %) of your personal tax' : 'Your latest projected liabilities'"
    >
        @if ($pendingNotice)
            <p class="mb-3 flex items-start gap-1.5 text-xs text-warning-700 dark:text-warning-400">
                ...
                <span>
                    {{ trans_choice(':count expense totalling CHF :gross awaiting confirmation is|:count expenses totalling CHF :gross awaiting confirmation are', $pendingNotice['count'], [
                        'count' => $pendingNotice['count'],
                        'gross' => number_format((float) $pendingNotice['gross'], 2, '.', "'"),
                    ]) }}
                    not included.
                    <a href="{{ $pendingNotice['url'] }}" class="font-semibold underline">Review</a>
                </span>
            </p>
        @endif
        ...
        @if ($estimation)
            <dl class="divide-y divide-gray-100 dark:divide-white/10">
                ...
                <div class="flex items-center justify-between pt-3">
                    <dt class="text-sm font-semibold text-gray-950 dark:text-white">Tax owed so far</dt>
            ...
            @if ($vatMessage)
                <p class="mt-3 text-xs text-gray-500 dark:text-gray-400">{{ $vatMessage }}</p>
            @endif

            <div class="mt-4 rounded-xl bg-gray-900 p-4 text-white ring-1 ring-inset ring-white/10 dark:bg-white/5">
                <div class="text-xs font-medium uppercase tracking-wide text-gray-400">Set aside monthly</div>
                <div class="mt-1 text-2xl font-semibold tabular-nums tracking-tight">{{ $money($estimation->projected_monthly_reserve) }}<span class="text-sm font-normal text-gray-400"> / month</span></div>
                <div class="mt-1 text-xs text-gray-400">From your projected full year of {{ $money($estimation->projected_total_tax) }}</div>
